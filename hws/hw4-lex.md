...
The second suspicious code pattern is hooking of an interrupt. This is a virus pattern from the DOS era, but similar assembly sequences exist on all modern platforms.  As discussed in the lectures concerning the boot sequence, one of the functions of the code in the boot sector is to load the IVT (interrupt vector table) into the memory that starts at address zero, where the DOS operating system expects to find it. The IVT is not normally changed after it is loaded; therefore, a user application that writes to these high addresses looks suspiciously like an interrupt-hooking virus. The address 4C in hexadecimal contains a pointer to DOS interrupt 21 (hex), which is often a target of viruses. So, a typical example of the conclusion of an interrupt 21h hook would be:
