...
- The volume/density problem from part 1 must compile to an executable
  named `volume`
- The tricky jump detection problem from part 2 must compile to an
...
- A PDF document called analysis.pdf with the analysis questions given
  above (in part 1) and their answers.
...
- File tricky.cpp with your C++ code for the `main()` function for the
  tricky jump detection code from part 2
- A Makefile that compiles the programs, as explained above -- make
  sure the executables are named `volume` and `tricky`!
